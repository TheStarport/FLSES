...
class FLSES extends Pluggable
{
	...
	public function sendCode2($player)
	{
		global $flhook;
		$flhook->connect();
		$code = "";
		...
		$flhook->fmsg($player,"<TRA data='0x16B20E01' mask='-1'/><TEXT>FLSES Access Code: </TEXT><TRA data='0xDFD58001' mask='-1'/><TEXT>".$code."</TEXT>");
		$flhook->fmsg($player,"<TRA data='0x16B20E00' mask='-1'/><TEXT>Enter this code into the form in the Web interface!</TEXT>");
		$flhook->fmsg($player,"<TRA data='0x16B20E00' mask='-1'/><TEXT>Do not give this code to other players as it will lead to abuse</TEXT>");
		$flhook->fmsg($player,"<TRA data='0x16B20E00' mask='-1'/><TEXT>This code was generated by </TEXT><TRA data='0x08A4FF00' mask='-1'/><TEXT>".$_SESSION["flses_name"]."</TEXT>");

		return($code);
	}
	
	/**
	* Envoie le code à un personnage en jeu.
	* @param player : Le nom du personnage
	...
	*/
	public function sendCode($player)
	{
		...
		$user_generator = $_SESSION["flses_name"];
		$flhook->sendCommand("fmsg $player <TRA data='0x16B20E01' mask='-1'/><TEXT>FLSES Access Code : </TEXT>");
		$flhook->sendCommand("fmsg $player <TRA data='0xDFD58001' mask='-1'/><TEXT>$code</TEXT>");
		$flhook->sendCommand("fmsg Kid <TRA data='0x16B20E00' mask='-1'/><TEXT>This code was generated by </TEXT><TRA data='0x08A4FF00' mask='-1'/><TEXT>$user_generator</TEXT>");
